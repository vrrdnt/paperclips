...
function normalizeCurrentTournament(s: GameState): void {
  ...
  if (ct.pendingYomi > 0 && ct.ticksRemaining === 0) ct.ticksRemaining = 1;
}
...
export function hydrateGameState(input: unknown): GameState {
  const loaded = validateSavedState(input);
  const initial = makeInitialState();
  const merged = { ...initial, ...structuredClone(loaded) };
  ...
  normalizeBattleNameNumbers(merged, input as LegacySavedState);
  normalizeDemandBoost(merged);
  normalizeMarketingCost(merged);
  normalizeInvestmentState(merged, input as LegacySavedState);
  ...
  for (const credit of ENDING_CREDITS.slice(0, Math.max(0, merged.milestoneFlag - 15))) displayMessage(merged, credit);
